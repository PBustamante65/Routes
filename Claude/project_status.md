...
- **As a result of the above fix, `Data/`'s own baseline outputs (`points-index.csv`, both matrices, `routes-solution.csv` + summary) were removed** — they had actually held the pipeline's 219-stop TEST-data results (per the old note here), not a clean run of the flat scripts. Running `Code/build_time_matrix.py` -> `Code/solve_routes.py` -> `Code/visualize_routes.py` again (against `Data/oxxo-stops.csv`, 218 stores) will regenerate the genuine flat baseline; this hasn't been done yet as of this write-up, so `Data/mapa_rutas.html` is stale until that run happens.
